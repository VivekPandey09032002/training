...
class PokemonServiceTests {
    ...
    void PokemonService_GetAllPokemon_ReturnsResponseDto() {
        ...
        PokemonResponse savePokemon = pokemonService.getAllPokemon(1, 1);

        Assertions.assertThat(savePokemon).isNotNull();
    }

    @Test
    void PokemonService_FindById_ReturnPokemonDto() {
        int pokemonId = 1;
    ...
    @Test
    void PokemonService_UpdatePokemon_ReturnPokemonDto() {
        int pokemonId = 1;
        Pokemon pokemon = Pokemon.builder().id(1).name("pikachu").type("electric").type("this is a type").build();
    ...
    @Test
    void PokemonService_DeletePokemonById_ReturnVoid() {
        int pokemonId = 1;
        Pokemon pokemon = Pokemon.builder().id(1).name("pikachu").type("electric").type("this is a type").build();
        when(pokemonRepository.findById(pokemonId)).thenReturn(Optional.ofNullable(pokemon));
        assert pokemon != null;
